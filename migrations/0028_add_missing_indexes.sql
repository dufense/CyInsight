...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tickets_assigned_to_status
  ON tickets (assigned_to, status) WHERE assigned_to IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_security_events_batch_id
  ON security_events (batch_id) WHERE batch_id IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_assets_last_seen
  ON assets (last_seen) WHERE last_seen IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_incidents_dedup_hash_tenant
