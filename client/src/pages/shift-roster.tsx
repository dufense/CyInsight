...
const SHIFT_TYPE_STYLES: Record<string, string> = {
  ...
  ai_24x7: "bg-cyan-500/10 text-cyan-700 dark:text-cyan-400",
};

const AI_SPEC_LABELS: Record<string, string> = {
...
function TeamMemberCard({
  ...
  member: TeamMember & { isAI?: boolean; aiSpecialization?: string; aiStats?: any };
  isMSS: boolean;
  onToggleActive: (id: number, isActive: boolean) => void;
}) {
  const isAI = !!(member as any).isAI || !!(member as any).is_ai;
  const spec = (member as any).aiSpecialization || (member as any).ai_specialization;
  const aiStats = (member as any).aiStats || (member as any).ai_stats;

  return (
    <Card className={`hover-elevate ${isAI ? "border-cyan-500/30 bg-cyan-500/5" : ""}`} data-testid={`card-member-${member.id}`}>
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              {isAI && <Bot className="w-4 h-4 text-cyan-500 shrink-0" />}
              <span className="text-sm font-medium" data-testid={`text-member-name-${member.id}`}>{member.name}</span>
              {isAI && (
                <Badge variant="outline" className="text-[10px] bg-cyan-500/10 text-cyan-700 dark:text-cyan-400 border-cyan-500/30">
                  AI Agent
                </Badge>
              )}
              <Badge
            ...
            {isAI && aiStats && (
              <div className="flex gap-2 mt-1 text-[10px] text-muted-foreground">
                <span>{aiStats.totalActions || 0} actions</span>
                {aiStats.ticketsResolved > 0 && <span>• {aiStats.ticketsResolved} tickets</span>}
                {aiStats.threatsFound > 0 && <span>• {aiStats.threatsFound} threats</span>}
              </div>
            )}
            {isAI && (
              <p className="text-[10px] text-cyan-600 dark:text-cyan-400 font-medium mt-0.5">24/7 Always On</p>
            )}
          </div>
          {isMSS && !isAI && (
            <Button
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-muted-foreground" data-testid={`text-shift-date-${shift.id}`}>
              {fmt.formatDate(shift.shiftDate)}
            </p>
          </div>
          {isMSS && (
            <Button
    ...
    <div className="h-full overflow-y-auto">
      <div className="space-y-6 p-6">
        <PageHero
          ...
          description={`Manage Implementation and MSS team schedules, on-call rotations, and shift assignments — ${currentTenant?.name}`}
        />
