              ...
              <div className="flex items-center gap-4 pt-1 flex-wrap">
                {service.msaStartDate && (
                  <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                    ...
                    {fmt.formatDate(service.msaStartDate)}
                    {service.msaEndDate && (
                      <>
                        {" - "}
                        {fmt.formatDate(service.msaEndDate)}
                      </>
                    )}
                  </span>
                )}
  ...
  return (
    <div className="space-y-6 p-6 overflow-y-auto h-full">
      <PageHero
        ...
        description={`Managed security services with MSA contracts and SLA compliance tracking — ${currentTenant?.name}`}
        stats={[
          { label: "services", value: services.length },
          { label: "active", value: services.filter(s => s.status === "active").length, accent: true },
        ]}
      />
